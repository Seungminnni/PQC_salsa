...
RA_DIR="$RUN_ROOT/ra_main"
AB_DIR="$RUN_ROOT/ab_binary_s10"
SECRET_SEED="${1:-1}"
INIT_SEED="${2:-}"
...
import numpy as np
s = np.load('/home/yu_mcc/PQC_salsa/runs/n70_q127_h3_1m/ab_binary_s10/secret.npy')
print(s.shape[1])
PY
)
...
run_one() {
  ...
  python train.py \
    ...
    --dump_path "$ROOT/runs" \
    --exp_name n70_q127_h3_1m \
    --exp_id "train_gpu_binary_s10_secret${SECRET_SEED}_init${init_seed}_paperlike" \
    --batch_size 128 \
    --epoch_size 20000 \
    --max_epoch 10000 \
    --eval_size 10000 \
