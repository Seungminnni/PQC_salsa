...
if [[ ! -s "$RUN_ROOT/ab_proposed_ternary_h10_s30/train.prefix" ]]; then
  python generate.py \
    --cpu true \
    --step Ab \
    ...
    --secret_type proposed_ternary_sparse \
    --sigma 3 \
    --gamma 2 \
    --epoch_size "$AB_EPOCH" \
    ...
    --dump_path "$ROOT/runs" \
    --exp_name "n${N}_q${Q}_h3_1m" \
    --exp_id ab_proposed_ternary_h10_s30
fi
