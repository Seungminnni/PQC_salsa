...
AB_DIR="$RUN_ROOT/ab_proposed_ternary_s10"
SECRET_SEED="${1:-0}"
INIT_SEED="${2:-}"
...
import numpy as np
s = np.load('/home/yu_mcc/PQC_salsa/runs/n30_q127_h3_1m/ab_proposed_ternary_s10/secret.npy')
print(s.shape[1])
PY
)
...
run_one() {
  ...
  python train.py \
    ...
    --exp_id "train_gpu_proposed_ternary_secret${SECRET_SEED}_init${init_seed}_paperlike" \
    --batch_size 128 \
    --epoch_size 20000 \
    --max_epoch 10000 \
    --eval_size 10000 \
