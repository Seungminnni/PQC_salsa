...
AB_DIR="$RUN_ROOT/ab_ternary_h10_s10"
SECRET_SEED="${1:-0}"
INIT_SEED="${2:-}"
EPOCH_SIZE="${3:-200000}"
...
import numpy as np
s = np.load('/home/yu_mcc/PQC_salsa/runs/n30_q127_h3_1m/ab_ternary_h10_s10/secret.npy')
print(s.shape[1])
PY
)
...
run_one() {
  ...
  python train.py \
    ...
    --exp_id "train_gpu_ternary_h10_secret${SECRET_SEED}_init${init_seed}_epoch${EPOCH_SIZE}" \
    --batch_size 128 \
...
if [[ -n "$INIT_SEED" ]]; then
  ...
  for init_seed in 0 1 2 3 4; do
    echo "=== Running secret_seed=${SECRET_SEED}, init_seed=${init_seed}, epoch_size=${EPOCH_SIZE} ==="
    run_one "$init_seed"
  done
fi
