    ...
    def run_bkz(self, return_dict, UT_or_AT, Ap):
        ...
        while True:
            ...
            if no_improve_rounds >= 3:
                logger.info('No stddev improvement for 3 BKZ rounds; returning current matrix to avoid infinite loop.')
                return Ap, False
...
class RA_Rb():
    ...
    @staticmethod
    def _sample_nonlinear_values(rng, sigma, size):
        raw = rng.normal(0.0, sigma, size=size)
        # Smooth odd nonlinearity with a light cubic tail.
        transformed = 1.2 * np.tanh(raw) + 0.3 * np.sin(1.6 * raw) + 0.12 * (raw ** 3) / (1.0 + np.abs(raw))
        vals = np.rint(sigma * transformed).astype(int)
        return np.clip(vals, -4, 4)

    def _sample_nonlinear_full_matrix(self, params, secret_size):
        # Lazy import so existing workflows remain unaffected unless nonlinear_full is used.
        from .secret_custom import build_secret_generator

        base_seed = params.env_base_seed if params.env_base_seed >= 0 else np.random.randint(1_000_000_000)
        generator = build_secret_generator(
            model_name="proposed_final",
            generator_type="proposed",
            n=self.N,
            run_seed=int(base_seed),
            params={"secret_sigma": float(params.sigma), "clipping_bound": 4},
        )
        cols = secret_size[1]
        secret = np.zeros(secret_size, dtype=int)
        for sample_index in range(cols):
            sample = generator.sample_with_metadata(sample_index)
            secret[:, sample_index] = sample.secret
        return secret
    ...
    def init_b(self, params):
        ...
        if os.path.isfile(os.path.join(params.secret_dir, 'secret.npy')):
            ...
            elif params.secret_type == "nonlinear":
                logger.info('Creating nonlinear secret')
                self.s = self._sample_nonlinear_values(rng, params.sigma, secret_size)
            elif params.secret_type == "nonlinear_full":
                logger.info('Creating nonlinear_full secret from external generator module')
                self.s = self._sample_nonlinear_full_matrix(params, secret_size)
            ...
            for h in range(params.min_hamming, params.max_hamming + 1):
                for seed_id in range(params.num_secret_seeds):
                    ...
                    # Gaussian / binomial may produce too many zeros. Ensure at least h non-zeros first.
                    if len(nonzeros) < h:
                        zeros = np.where(self.s[:, column] == 0)[0]
                        add_count = h - len(nonzeros)
                        add_idxs = np.random.choice(zeros, size=add_count, replace=False)
                        if params.secret_type == "ternary":
                            self.s[add_idxs, column] = np.random.choice([-1, 1], size=add_count)
                        elif params.secret_type == "gaussian":
                            vals = np.random.normal(0, params.sigma, size=add_count).round().astype(int)
                            while np.any(vals == 0):
                                zero_mask = vals == 0
                                vals[zero_mask] = np.random.normal(0, params.sigma, size=zero_mask.sum()).round().astype(int)
                            self.s[add_idxs, column] = vals
                        elif params.secret_type == "nonlinear":
                            vals = self._sample_nonlinear_values(rng, params.sigma, add_count)
                            while np.any(vals == 0):
                                zero_mask = vals == 0
                                vals[zero_mask] = self._sample_nonlinear_values(rng, params.sigma, int(zero_mask.sum()))
                            self.s[add_idxs, column] = vals
                        elif params.secret_type == "nonlinear_full":
                            vals = self._sample_nonlinear_values(rng, params.sigma, add_count)
                            while np.any(vals == 0):
                            ...
                            vals = np.random.binomial(params.gamma, 0.5, add_count) - np.random.binomial(params.gamma, 0.5, add_count)
                            while np.any(vals == 0):
                                zero_mask = vals == 0
                                vals[zero_mask] = np.random.binomial(params.gamma, 0.5, zero_mask.sum()) - np.random.binomial(params.gamma, 0.5, zero_mask.sum())
                            self.s[add_idxs, column] = vals
                        else:
                            self.s[add_idxs, column] = 1
                        nonzeros = np.where(self.s[:, column] != 0)[0]
